...
{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../figures/Markov_Chain_State_Classification_Guide.png}
...
\begin{frame}{Random Walk with Absorbing Boundaries}
\protect\phantomsection\label{random-walk-with-absorbing-boundaries-1}
...
\protect\phantomsection\label{expected-time-to-absorption}
Define the absorption time

\[
T=\inf\{n\geq0:X_n\text{ belongs to a closed class}\}.
\]

\begin{itemize}
\tightlist
\item
  \(T\) is the number of transitions until the process enters some
  closed class.
\item
...
\[
v_i=\mathbb{E}_i[T]=\mathbb{E}[T\mid X_0=i].
\]

\begin{itemize}
\tightlist
\item
  Thus, \(v_i\) is the expected number of transitions until absorption,
  starting from state \(i\).
\end{itemize}
\end{frame}

\begin{frame}{First-Step Analysis}
\protect\phantomsection\label{first-step-analysis}
Starting from transient state \(i\), consider the first transition.

\begin{itemize}
\tightlist
\item
  One transition occurs immediately.
\item
  After this transition, suppose \(X_1=j\).
\item
  The expected remaining time is \(v_j\).
\end{itemize}
...
The \(1\) accounts for the first transition.

The summation gives the expected remaining time after the first
transition.
\end{frame}

\begin{frame}{Matrix Formulation}
\protect\phantomsection\label{matrix-formulation-1}
Let

\[
\mathbf{v}=(v_1,\ldots,v_m)^\mathsf{T},
\qquad
...
\[
\boxed{
(I-P)\mathbf{v}=\mathbf{1}.
}
\]

Here, \(\mathbf{v}\) is treated as a column vector.
\end{frame}

\begin{frame}{Boundary Conditions}
\protect\phantomsection\label{boundary-conditions-1}
For a state already belonging to a closed class,

\[
\boxed{v_i=0.}
\]

The process is already absorbed, so no further transitions are required.
...
\tightlist
\item
  \textbf{Closed-class states:} \(v_i=0\).
\item
  \textbf{Transient states:} \(v_i\) is determined by the first-step
  equations.
\end{itemize}

The resulting system gives the expected time to absorption from each
transient state.
\end{frame}
...
{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../figures/Stationary_vs._Limiting_Distributions_Analogy.png}
...
Thus, the NCD system is \(\boxed{\text{irreducible and aperiodic}.}\)
\end{frame}
...
A periodic chain can visit every state indefinitely, but its state
probabilities may oscillate rather than converge.

For example, consider the Markov chain with state space
\(\mathcal{S}={0,1}\) and transition matrix
...
but if \(X_0=0\), the distribution alternates between
\((1,0) \quad\text{and}\quad (0,1).\)

Thus, \(\boldsymbol{\pi}^{(n)}\) does not converge.
\end{frame}

\begin{frame}{Understanding Periodic Markov Chains}
\protect\phantomsection\label{understanding-periodic-markov-chains}
\begin{figure}[H]

{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../figures/Understanding_Periodic_Markov_Chains.png}

}

\caption{Understanding Periodic Markov Chains}
...
\begin{frame}{State Classification Roadmap}
\protect\phantomsection\label{state-classification-roadmap-1}
\begin{figure}[H]

{\centering \includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../figures/Finite_Markov_Chain_Behavior_Guide.png}
...
\begin{frame}{Classification of Long-Run Behaviour}
\protect\phantomsection\label{classification-of-long-run-behaviour}
For a finite DTMC, identify the communicating classes and closed classes
first.
...
\tightlist
\item
  \textbf{Irreducible:} all states communicate.
\item
  \textbf{Reducible:} there is more than one communicating class.
\item
  A closed communicating class is irreducible.
\end{itemize}

For an irreducible chain, we then check its period:
...
\begin{frame}{Irreducible Chains}
\protect\phantomsection\label{irreducible-chains}
An irreducible chain has one communicating class.
...
The \(n\)-step distribution converges to the unique stationary
distribution:
...
The distribution generally oscillates between states:
...
The chain still has a unique stationary distribution, but the \(n\)-step
distribution does not converge to it.
\end{frame}

\begin{frame}{Reducible Chains}
\protect\phantomsection\label{reducible-chains}
A reducible chain has more than one communicating class.

\textbf{One closed class}

Transient states eventually enter the closed class.

\[
\text{Transient states}
\longrightarrow
\boxed{\text{Closed class}}
\]

The long-run behaviour is determined by the closed class.

If the closed class is aperiodic, the distribution converges to its
stationary distribution.
\end{frame}

\begin{frame}{Reducible Chains: Multiple Closed Classes}
\protect\phantomsection\label{reducible-chains-multiple-closed-classes}
A reducible chain may have several closed communicating classes.
...
The process eventually enters one of the closed classes.

\begin{itemize}
\tightlist
\item
  The absorption probabilities determine which closed class is reached.
\item
  Each closed class has its own stationary distribution.
\item
  The period of each closed class determines its long-run behaviour.
\end{itemize}

Thus, for a reducible chain, \textbf{analyse each closed communicating
class separately}.
\end{frame}
...
\begin{frame}{Example: A Two-State Markov Chain}
\protect\phantomsection\label{example-a-two-state-markov-chain}
Consider the Markov chain with state space \(S=\{1,2\}\) and transition
matrix
...
The chain is \textbf{finite, irreducible, and aperiodic}.
...
The limiting distribution exists and is independent of the initial
state.
\end{frame}

\begin{frame}{Example: A Two-State Markov Chain}
\protect\phantomsection\label{example-a-two-state-markov-chain-1}
...
\begin{frame}{Example: A Two-State Markov Chain}
\protect\phantomsection\label{example-a-two-state-markov-chain-2}
The limiting distribution is also the unique stationary distribution.
...
Thus, for any initial distribution \(\boldsymbol{\pi}^{(0)}\),
...
\begin{frame}{Example: A Two-State Markov Chain}
\protect\phantomsection\label{example-a-two-state-markov-chain-3}
This example illustrates the theorem in a simple setting:

\begin{itemize}
\tightlist
\item
  The chain is \textbf{finite}.
\item
  The chain is \textbf{irreducible}.
\item
  The chain is \textbf{aperiodic}.
\item
  Therefore, a unique stationary distribution exists.
\item
  The distribution at time \(n\) converges to this stationary
  distribution.
\end{itemize}
...
The result does not depend on the initial state.
\end{frame}

\begin{frame}{Example: Long-run Behaviour of the NCD System}
\protect\phantomsection\label{example-long-run-behaviour-of-the-ncd-system}
Consider the NCD Markov chain introduced earlier.

Its state space is finite, and the chain is \textbf{irreducible and
aperiodic}.

Therefore, the theorem applies, and the limiting distribution exists and
...
\begin{frame}{Example: Long-run Behaviour of the NCD System}
\protect\phantomsection\label{example-long-run-behaviour-of-the-ncd-system-1}
For every initial state \(i\),
...
Thus, the long-run probability of being in state \(j\) is \(\pi_j\),
...
\begin{frame}{Example: Long-run Behaviour of the NCD System}
\protect\phantomsection\label{example-long-run-behaviour-of-the-ncd-system-2}
For \(p=0.8\),
...
The rows of \(P^n\) begin to become similar as \(n\) increases.
\end{frame}

\begin{frame}{Example: Long-run Behaviour of the NCD System}
\protect\phantomsection\label{example-long-run-behaviour-of-the-ncd-system-3}
For \(n=8\),
...
The rows are already close to one another.

As \(n\) increases, each row converges to the same limiting
distribution.
\end{frame}

\begin{frame}{Example: Long-run Behaviour of the NCD System}
\protect\phantomsection\label{example-long-run-behaviour-of-the-ncd-system-4}
...
\begin{frame}{Example: Long-run Behaviour of the NCD System}
\protect\phantomsection\label{example-long-run-behaviour-of-the-ncd-system-5}
This example illustrates the theorem for an actuarial model.

Since the NCD chain is \textbf{finite, irreducible, and aperiodic},
...
Thus, in the long run, the probability of each NCD level is determined
by \(\boldsymbol{\pi}\), not by the initial NCD level.
\end{frame}
