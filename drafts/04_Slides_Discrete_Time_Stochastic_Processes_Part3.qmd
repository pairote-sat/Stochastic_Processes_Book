...
* $T$ is the number of transitions until the process enters some closed class.
...
* Thus, $v_i$ is the expected number of transitions until absorption, starting from state $i$.
...
The summation gives the expected remaining time after the first transition.
...
* **Closed-class states:** $v_i=0$.
* **Transient states:** $v_i$ is determined by the first-step equations.

The resulting system gives the expected time to absorption from each transient state.
...
A periodic chain can visit every state indefinitely, but its state probabilities may oscillate rather than converge.
...
Its stationary distribution is $\boldsymbol{\pi}=\left(\frac12,\frac12\right),$

but if $X_0=0$, the distribution alternates between $(1,0) \quad\text{and}\quad (0,1).$

Thus, $\boldsymbol{\pi}^{(n)}$ does not converge.



## Understanding Periodic Markov Chains

![Understanding Periodic Markov Chains](../figures/Understanding_Periodic_Markov_Chains.png){width=75% fig-align="center"}
...
![Markov Chain State Classification Guide](../figures/Finite_Markov_Chain_Behavior_Guide.png){width=75% fig-align="center"}


## Classification of Long-Run Behaviour

For a finite DTMC, identify the communicating classes and closed classes first.
...
* **Reducible:** there is more than one communicating class.
...
The $n$-step distribution converges to the unique stationary distribution:
...
The chain still has a unique stationary distribution, but the $n$-step distribution does not converge to it.
...
If the closed class is aperiodic, the distribution converges to its stationary distribution.

## Reducible Chains: Multiple Closed Classes
...
Thus, for a reducible chain, **analyse each closed communicating class separately**.
...
The limiting distribution exists and is independent of the initial state.
...
* The distribution at time $n$ converges to this stationary distribution.
...
Its state space is finite, and the chain is **irreducible and aperiodic**.

Therefore, the theorem applies, and the limiting distribution exists and is the unique stationary distribution.
...
Thus, the long-run probability of being in state $j$ is $\pi_j$, independently of the initial state.
...
As $n$ increases, each row converges to the same limiting distribution.
...
Since the NCD chain is **finite, irreducible, and aperiodic**,
...
Thus, in the long run, the probability of each NCD level is determined by $\boldsymbol{\pi}$, not by the initial NCD level.
