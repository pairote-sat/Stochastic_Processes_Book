...
::: {.callout-note #nte-expected-absorption-time}
...
$$
T=\inf\{n\geq 0:X_n\text{ belongs to a closed class}\}.
$$

Thus, $T$ is the number of transitions until the process enters some closed class.

For a transient state $i$, define

$$
v_i=\mathbb{E}_i[T]
=\mathbb{E}[T\mid X_0=i].
$$

In words, $v_i$ is the expected number of transitions until absorption when the process starts from state $i$.

Conditioning on the state after the first transition gives
...
The $1$ accounts for the first transition, while the summation gives the expected remaining time after reaching state $j$.
...
where $\mathbf{v}=(v_1,\ldots,v_m)^\mathsf{T}$ and $\mathbf{1}$ is an $m$-dimensional column vector of ones.

For states belonging to a closed class,

$$
v_i=0,
$$

since the process is already absorbed.
:::
...
::: {.callout-note #nte-long-run-classification}

## Classification of Long-Run Behaviour

For a finite DTMC, the long-run behaviour is determined by its communicating classes, closed classes, and their periods.

**1. Irreducible chains**

An irreducible chain has only one communicating class: all states communicate with each other.

* **Aperiodic:** The common period is $1$. The $n$-step distribution converges to the unique stationary distribution.
* **Periodic:** The common period is $d>1$. The distribution generally oscillates between states and does not converge to a single limiting distribution.

Thus, an irreducible chain is either **aperiodic** or **periodic**.

**2. Reducible chains**

A reducible chain has more than one communicating class. Some classes may be transient, while closed classes determine the long-run behaviour.

* **One closed class:** All transient states eventually enter the closed class. The long-run behaviour is then determined by this closed class.
* **Multiple closed classes:** The process eventually enters one of the closed classes. The long-run behaviour depends on which closed class is reached, with the absorption probabilities determining the probabilities of reaching the different classes.

Each closed communicating class is itself irreducible and can be either **aperiodic** or **periodic**.

**Summary**

The classification can be viewed as
...
&
\begin{cases}
\text{One closed class}\\
\text{Multiple closed classes}
\end{cases}
\end{cases}
}
$$

For a reducible chain, the period should be considered separately for each closed communicating class.
:::
